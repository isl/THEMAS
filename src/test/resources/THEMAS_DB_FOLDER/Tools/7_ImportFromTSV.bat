...
SET Mode=ImportFromTSV
SET inputFile=%BaseNeo4jFolderPath%\TSVs\System\Generic.tsv
SET containsGenericDefinitions=true

SET SystemOutFilePath="%BaseInputOutpoutPath%\OfflineToolsClassOutput.txt"


cd "%BaseTHEMASwebappPath%\WEB-INF\classes"
"%JAVA_BIN_PATH%\java.exe"  -Xmx2048M -cp ".;%libFolder%\*;" -Dfile.encoding=UTF8 Admin_Thesaurus.OfflineToolsClass %Mode% "%BaseTHEMASwebappPath%" "%inputFile%" %containsGenericDefinitions% 1> %SystemOutFilePath% 2>&1
