...
SET Mode=ExportToXML

REM for export of all thesauri
SET ThesaurusName=XXXXXX 

REM for export of specific thesaurus named TEST
REM SET ThesaurusName=TEST 
SET ExportFolderPath="%BaseInputOutpoutPath%"

SET SystemOutFilePath="%BaseInputOutpoutPath%\OfflineToolsClassOutput.txt"


cd "%BaseTHEMASwebappPath%\WEB-INF\classes"
"%JAVA_BIN_PATH%\java.exe"  -Xmx2048M -cp ".;%libFolder%\*;" -Dfile.encoding=UTF8 Admin_Thesaurus.OfflineToolsClass %Mode% "%BaseTHEMASwebappPath%" %ThesaurusName% %ExportFolderPath% 1> %SystemOutFilePath% 2>&1
