...
SET Mode=ImportFromXML

SET ThesaurusName=TEST
SET InputXMLFile="%BaseInputOutpoutPath%\TESTIMPORT.xml"
SET LogFilePath="%BaseInputOutpoutPath%\importFromXMLLog.xml"

SET SystemOutFilePath="%BaseInputOutpoutPath%\OfflineToolsClassOutput.txt"


cd "%BaseTHEMASwebappPath%\WEB-INF\classes"
"%JAVA_BIN_PATH%\java.exe"  -Xmx2048M -cp ".;%libFolder%\*;" -Dfile.encoding=UTF8 Admin_Thesaurus.OfflineToolsClass %Mode% "%BaseTHEMASwebappPath%" %ThesaurusName% %InputXMLFile% %LogFilePath% 1> %SystemOutFilePath% 2>&1
