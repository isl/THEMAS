...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:fn="http://www.w3.org/2005/02/xpath-functions" xmlns:xdt="http://www.w3.org/2005/02/xpath-datatypes">
	<!-- _____________________________________________________________________________
			TEMPLATE: DisplayStatisticsAndPagingInfo
			FUNCTION: displays the paging mechanism for the query results
	      _____________________________________________________________________________ -->
    <xsl:template name="DisplayStatisticsAndPagingInfo_Terms">
        ...
        <tr width="100%">            
            ...
            <td class="PagingInfo" >                
                ...
                <xsl:text> </xsl:text>&#160;&#160;&#160;&#160;
                <xsl:call-template name="SearchResultsPaging_Terms">
                ...
                &#160;&#160;&#160;&#160; 
                        ...
                        <a href="#">
                            <xsl:attribute name="onClick">prepareResults('SearchResults_Terms','','','true');</xsl:attribute>
	...
	<!-- _____________________________________________________________________________
	      ...
	      _____________________________________________________________________________ -->
    <xsl:template name="SearchResultsPaging_Terms">
        <xsl:param name="idsuffix"/>
	...
	<xsl:param name="paginglocale"/>
        ...
        <xsl:if test="$pagingFirst > 1">
            ...
            <a>
                ...
                <!--<xsl:attribute name="onclick">
                    getResults('15','SearchResults_Terms','1');
                </xsl:attribute>-->
	    ...
	    <!-- previous ARROW -->
            <a>
                ...
                <xsl:attribute name="onclick">DisplayPleaseWaitScreen(true);</xsl:attribute>
               <!-- <xsl:attribute name="onclick">
                    getResults('15','SearchResults_Terms',<xsl:value-of select="$pre_start"/>);
                </xsl:attribute>-->
        ...
        <xsl:if test="$pagingLast &lt; $pagingQueryResultsCount">
            ...
            <a>
                ...
                <xsl:attribute name="onclick">DisplayPleaseWaitScreen(true);</xsl:attribute>
                <!--<xsl:attribute name="onclick">
                    getResults('15','SearchResults_Terms',<xsl:value-of select="$pagingLast + 1"/>);
                </xsl:attribute>-->
            ...
            <a>
                ...
                <xsl:attribute name="onclick">DisplayPleaseWaitScreen(true);</xsl:attribute>
                <!--<xsl:attribute name="onclick">
                    getResults('15','SearchResults_Terms',<xsl:value-of select="$end_arrow_parameter"/>);
                </xsl:attribute>-->
        ...
        <xsl:if test="$pagingQueryResultsCount &gt; $pagingListStep">
            ...
            <input name="go_to_specific_page_input_term" style="font-size: 8pt; width: 25pt">
                <xsl:attribute name="id">
                    <xsl:text>go_to_specific_page_input_term_</xsl:text><xsl:value-of select="$idsuffix"/>                    
                </xsl:attribute>
               <xsl:attribute name="onKeyPress">                    
                    if(event.keyCode == 13) {
						DisplayPleaseWaitScreen(true);
						checkPageNumber('SearchResults_Terms', '<xsl:value-of select="$pagingListStep"/>',document.getElementById('go_to_specific_page_input_term_<xsl:value-of select="$idsuffix"/>').value);
					}
                </xsl:attribute>
            </input>&#160;
