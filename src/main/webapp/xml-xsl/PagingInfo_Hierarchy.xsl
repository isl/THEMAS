...
<!--
    Document   : PagingInfo_Hierarchy.xsl
    Created on : 11 Ιούλιος 2008, 11:30 πμ
    Author     : tzortzak
    Description:
...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:template  name="DisplayStatisticsAndPagingInfo_Hierarchies">
        ...
        <tr width="100%">
            ...
            <td class="PagingInfo">
                ...
                <xsl:text> </xsl:text>&#160;&#160;&#160;&#160;
                <xsl:call-template name="SearchResultsPaging_Hierarchies">
                ...
                &#160;&#160;&#160;&#160; 
                <a href="#">
                    <xsl:attribute name="onClick">prepareResults('SearchResults_Hierarchies','','','true');</xsl:attribute>
                ...
                <a href="#">
                    <xsl:attribute name="onClick">downloadFile('SearchResults_Hierarchies','','','true');</xsl:attribute>
                    <img  height="16" width="16" border="0" class="img_link" >
                                <xsl:attribute name="src">
	...
	<!-- _____________________________________________________________________________
	      ...
	      _____________________________________________________________________________ -->
    <xsl:template name="SearchResultsPaging_Hierarchies">
        <xsl:param name="idsuffix" />
	...
	<!-- previous and start ARROW -->
        <xsl:if test="$pagingFirst > 1">
            ...
            <a>
                ...
                <!--<xsl:attribute name="onclick">
                    getResults('18','SearchResults_Hierarchies', '1');
                </xsl:attribute>-->
            ...
            <a>
                ...
                <xsl:attribute name="onclick">DisplayPleaseWaitScreen(true);</xsl:attribute>
                <!--<xsl:attribute name="onclick">
                    getResults('18','SearchResults_Hierarchies', <xsl:value-of select="$pre_start"/>);
                </xsl:attribute>-->
        ...
        <xsl:if test="$pagingLast &lt; $pagingQueryResultsCount">
            ...
            <a>
                ...
                <xsl:attribute name="onclick">DisplayPleaseWaitScreen(true);</xsl:attribute>
                <!--<xsl:attribute name="onclick">
                    getResults('18','SearchResults_Hierarchies', <xsl:value-of select="$pagingLast + 1"/>);
                </xsl:attribute>-->
            ...
            <a>
                ...
                <xsl:attribute name="onclick">DisplayPleaseWaitScreen(true);</xsl:attribute>
                <!--<xsl:attribute name="onclick">
                    getResults('18','SearchResults_Hierarchies', <xsl:value-of select="$end_arrow_parameter"/>);
                </xsl:attribute>-->
	...
	<!-- go to specific page - part -->
        <xsl:if test="$pagingQueryResultsCount &gt; $pagingListStep">
            ...
            <input name="go_to_specific_page_input_hierarchy" style="font-size: 8pt; width: 25pt">
                <xsl:attribute name="id">
                    <xsl:text>go_to_specific_page_input_hierarchy_</xsl:text><xsl:value-of select="$idsuffix"/>                    
                </xsl:attribute>
                <xsl:attribute name="onKeyPress">
                    if(event.keyCode == 13) {
						DisplayPleaseWaitScreen(true);
						checkPageNumber('SearchResults_Hierarchies', '<xsl:value-of select="$pagingListStep"/>',document.getElementById('go_to_specific_page_input_hierarchy_<xsl:value-of select="$idsuffix"/>').value);
					}
                </xsl:attribute>
            </input>&#160;
