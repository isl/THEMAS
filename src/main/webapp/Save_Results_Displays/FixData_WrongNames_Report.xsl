...
<!-- 
 ...
 -->
<!--
    Document   : FixData_WrongNames_Report.xsl
    Created on : 28 Νοέμβριος 2008, 3:59 μμ
    Author     : tzortzak
    Description:
...
<xsl:stylesheet version="2.0" 
    ...
    <xsl:template match="/" >
        ...
        <xsl:variable name="localespecific" select="document($pathToSaveScriptingAndLocale)/root/fixdata/wrongnames"/>
                    ...
                    <xsl:otherwise>
                        <table width="100%" >
                            <tr>
                                ...
                                <td align="center">
                                    <strong>
                                        <xsl:call-template name="getTranslationMessage"> <xsl:with-param name="targetLangElements" select="$localespecific/kind/option"/> <xsl:with-param name="targetLang" select="$lang"/> <xsl:with-param name="disableEscape" select="'no'"/> </xsl:call-template>    
                                    </strong>
                                </td>
                                <td align="center">
                                    <strong>
                                        <xsl:call-template name="getTranslationMessage"> <xsl:with-param name="targetLangElements" select="$localespecific/oldname/option"/> <xsl:with-param name="targetLang" select="$lang"/> <xsl:with-param name="disableEscape" select="'no'"/> </xsl:call-template>    
                                    </strong>
                                </td>
                                <td align="center">
                                    <strong>
                                        <xsl:call-template name="getTranslationMessage"> <xsl:with-param name="targetLangElements" select="$localespecific/newname/option"/> <xsl:with-param name="targetLang" select="$lang"/> <xsl:with-param name="disableEscape" select="'no'"/> </xsl:call-template>    
                                    </strong>
                                </td>
                                <td align="center">
                                    <strong>
                                        <xsl:call-template name="getTranslationMessage"> <xsl:with-param name="targetLangElements" select="$localespecific/newnameexists/option"/> <xsl:with-param name="targetLang" select="$lang"/> <xsl:with-param name="disableEscape" select="'no'"/> </xsl:call-template>    
                                    </strong>
                                </td>
                            </tr>
                            <tr>
                            ...
                            <xsl:for-each select="//concept">
                                <xsl:sort select="./oldname"/>
                                <tr>
                                    <td onMouseOver="this.bgColor = '#F2F2F2'" onMouseOut="this.bgColor = '#FFFFFF'" bgcolor="#FFFFFF" >
                                        <xsl:value-of select="position()"/>
                                    </td>
                                    <td onMouseOver="this.bgColor = '#F2F2F2'" onMouseOut="this.bgColor = '#FFFFFF'" bgcolor="#FFFFFF" >
                                        <xsl:value-of select="./kind"/><!-- FACET / TERM / HIERARCHY / UF / ALT / ENGLISH WORD / SCOPENOTE etc.-->
                                    </td>
                                    <td onMouseOver="this.bgColor = '#F2F2F2'" onMouseOut="this.bgColor = '#FFFFFF'" bgcolor="#FFFFFF" >
                                        <xsl:value-of select="./oldname" disable-output-escaping="yes"/>
                                    </td>
                                    <td onMouseOver="this.bgColor = '#F2F2F2'" onMouseOut="this.bgColor = '#FFFFFF'" bgcolor="#FFFFFF" >
                                        <xsl:value-of  select="./newname"/>
                                    </td>
                                    <td onMouseOver="this.bgColor = '#F2F2F2'" onMouseOut="this.bgColor = '#FFFFFF'" bgcolor="#FFFFFF" >
                                        <xsl:value-of  select="./newnameexists"/>
                                    </td>
                                </tr>
                            </xsl:for-each>
                        </table>
