...
public class MoveToHierarchyResults extends ApplicationBasicServlet {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            String MoveΤοHierarchy = u.getDecodedParameterValue(request.getParameter("DestinationHierList"));
            ...
            if (action.compareTo("moveToHier") == 0) {
                finalResult = this.moveToHierAction(SessionUserInfo, Q, TA, sis_session, tms_session, TargetTermName, MoveToHierarchyOption, MoveBTterm, MoveToHierarchyResultsMessage, errorMsg, pathToErrorsXML, MoveFromHierarchy, MoveΤοHierarchy);
    ...
    private boolean moveToHierAction(UserInfoClass SessionUserInfo, QClass Q, TMSAPIClass TA, IntegerObject sis_session, IntegerObject tms_session,
            ...
            String MoveFromHierarchy,String MoveΤοHierarchy) {
        ...
        if (consistencyChecks.check_Move_To_Hier_Consistencies(SessionUserInfo, Q, sis_session, dbGen, prefix, errorMsg, pathToErrorsXML,
            ...
            if (MoveToHierarchyOption.compareTo("MOVE_NODE_ONLY") == 0 || MoveToHierarchyOption.compareTo("MOVE_NODE_AND_SUBTREE") == 0) {
                if (MoveToHierarchyAction(SessionUserInfo, Q, TA, sis_session, tms_session, dbGen, TargetTermName, MoveFromHierarchy, MoveΤοHierarchy, MoveBTterm, MoveToHierarchyOption, user, MoveToHierarchyResultsMessage) == false) {
    ...
    ----------------------------------------------------------------------*/
    boolean MoveToHierarchyAction(UserInfoClass SessionUserInfo, QClass Q, TMSAPIClass TA, IntegerObject sis_session, IntegerObject tms_session, DBGeneral dbGen, String TargetTermName, String MoveFromHierarchy, String MoveΤοHierarchy, String MoveBTterm, String MoveToHierarchyOption, String user, StringObject MoveToHierarchyResultsMessage) {
        ...
        if (ret == TMSAPIClass.TMS_APISucc) { // SUCCESS
            ...
            if (MoveToHierarchyResultsMessage.getValue().compareTo("") == 0) {
                MoveToHierarchyResultsMessage.setValue("Η μετακίνηση του όρου \"" + TargetTermName + "\", ολοκληρώθηκε επιτυχώς.");
                return true;
            } else {
    ...
    boolean DeleteBTAction(String selectedThesaurus, QClass Q, IntegerObject sis_session, TMSAPIClass TA, IntegerObject tms_session, DBGeneral dbGen, String TargetTermName, String TargetBTforDeletion, String user, StringObject MoveToHierarchyResultsMessage) {
        ...
        */
        ...
        if (ret == QClass.APISucc) { // SUCCESS
            ...
            Q.free_all_sets();
            if (MoveToHierarchyResultsMessage.getValue().compareTo("") == 0) {
                MoveToHierarchyResultsMessage.setValue("Η διαγραφή του όρου \"" + TargetBTforDeletion + "\" ως ευρύτερου του όρου \"" + TargetTermName + "\" ολοκληρώθηκε επιτυχώς.");
            ...
            MoveToHierarchyResultsMessage.setValue("Η διαγραφή του όρου \"" + TargetBTforDeletion + "\" ως ευρύτερου του όρου \"" + TargetTermName + "\" απέτυχε.");
            Q.free_all_sets();
